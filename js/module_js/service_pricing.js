...
var service_price_load_flag = true;

$(document).on('submit', 'form#add_service_price_form', function (e) {

    e.preventDefault();
    dataString = $("#add_service_price_form").serialize();
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $("#main_body").load("module/administration/service_pricing.html", function () {

                get_service_prices();
                get_services();
                $(".update_service_price_div").hide();
                $(".add_service_price_div").hide();
                $(".service_price_table_div").show();
            });
...
$(".add_service_price_link").click(function () {
    $(".update_service_price_div").hide();
    $(".add_service_price_div").show();
    $(".service_price_table_div").hide();
});
...
$(document).on('submit', 'form#update_service_price_form', function (e) {

    e.preventDefault();
    dataString = $("#update_service_price_form").serialize();
...
$(document).on('click', ".edit_btn", function () {
    ...
    var b_price_list_id = $(this).closest('tr').find('input[name="b_price_list_id"]').val();

    $.ajax({
        url: "" + api_url + "/v_service_prices?filter[where][b_price_list_id]=" + b_price_list_id,
        type: "GET",
        data: {
        },
        dataType: "JSON",
        success: function (data) {
            ...
            $.each(data, function (value, i) {
                ...
                var service_name = i.service_name;
                ...
                document.getElementById("update_service_price_amount").value = amount;
                document.getElementById("update_service_price_selling_price").value = selling_price;
...
//
    ...
    swal({
        ...
        if (result) {
            ...
            $.ajax({
                type: 'DELETE',
                url: "" + api_url + "/b_price_lists/" + b_price_list_id,
                crossDomain: true,
                success: function (data) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (response_type == 'object') {
                ...
                var table = '<table class=" service_prices_table   table table-striped table-bordered dt-responsive nowrap" id="service_prices_table" cellspacing="0" width="100%">\n\ <thead>\n\
                                    <tr> <th> No</th>  <th> Name</th> \n\
                                    <th>Code No</th> <th>Group Name </th>  <th>Amount</th> <th>Is Current</th> <th>Is Active</th> <th>Action</th> <th>Edit</th> </tr>\n\
                                   </thead>  <tbody id="results_tbody" class="results_tbody"> </tbody> </table>';

                $(".service_price_results_div").empty();
                $('.service_price_results_div').append(table);
                var no = 1;
                $.each(data, function (value, i) {
                    var service_name = i.service_name;
                    var productgroup_name = i.productgroup_name;
               ...
               <td>" + code + "</td>\n\
                ...
                $('.service_prices_table').DataTable({});
                //$(".service_price_results_div").append();
                    ...
                    closeOnCancel: false
                        ...
                        function (isConfirm) {
                            if (isConfirm) {
                                $("#main_body").empty();
                                $("#modal_loading").modal("show");
                                $("#main_body").load("module/administration/service_pricing.html");
                            } else {
...
function service_price_click() {

    $(document).on('click', ".add_service_price", function () {
        $("#id_sname").empty();
        $(".add_service_price_div").show();
        $(".service_price_table_div").hide();
    });
...
function get_services() {
    ...
    var settings = {
        "async": true,
        "crossDomain": true,
        "url": "" + api_url + "/m_products?filter[where][isservice]=Y",
        "method": "GET",
        "headers": {
